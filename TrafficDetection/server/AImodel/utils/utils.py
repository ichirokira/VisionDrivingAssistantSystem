    ...
    for i in range(len(preds)):
        if len(preds[i]['rois']) == 0:
            ...
            if metas is float:
                ...
                preds[i]['rois'][:, [0, 2]] = preds[i]['rois'][:,
                                                               [0, 2]] / (new_w / old_w)
                preds[i]['rois'][:, [1, 3]] = preds[i]['rois'][:,
                                                               [1, 3]] / (new_h / old_h)
    return preds
...
def plot_one_box(img, coord, label=None, score=None, color=None, line_thickness=None):
    ...
    if label:
        ...
        s_size = cv2.getTextSize(
            str('{:.0%}'.format(score)), 0, fontScale=float(tl) / 3, thickness=tf)[0]
